...
DROP TABLE IF EXISTS SANDBOX.DS.EXAMPLE_MODEL2_ALLCENTERS1; 
CREATE temp TABLE  SANDBOX.DS.EXAMPLE_MODEL2_ALLCENTERS1 AS 
...
from (select ZIP,CENTER,CENTER_ZIP_CODE,DISTANCE from EXAMPLE_ALLUS_DISTANCE) A
...
select count(*) from EXAMPLE_MODEL2_ALLCENTERS1;



DROP TABLE IF EXISTS EXAMPLE_MODEL2_ALLCENTERS_UNIQUE_ID;
create table EXAMPLE_MODEL2_ALLCENTERS_UNIQUE_ID AS 
select *, 1000000000+ROW_NUMBER() OVER(ORDER BY CENTER DESC) AS UNIQUE_ID 
from SANDBOX.DS.EXAMPLE_MODEL2_ALLCENTERS1;
...
DROP TABLE IF EXISTS SANDBOX.DS.JW_EXAMPLE_TEMP_NATIONAL; 
CREATE TEMP TABLE SANDBOX.DS.JW_EXAMPLE_TEMP_NATIONAL AS
SELECT  *,
...
FROM EXAMPLE_MODEL2_ALLCENTERS_UNIQUE_ID;



DROP TABLE IF EXISTS SANDBOX.DS.EXAMPLE_MODEL2CENTERS_SCORE_20200225; 
CREATE temp TABLE  SANDBOX.DS.EXAMPLE_MODEL2CENTERS_SCORE_20200225 AS 
...
SELECT  *,  1 +(1 * GENDER11) +
 ...
 AS NATIONAL_LOGIT 
FROM    SANDBOX.DS.JW_EXAMPLE_TEMP_NATIONAL);




SELECT * FROM EXAMPLE_MODEL2CENTERS_SCORE_20200225 LIMIT 100;


DROP TABLE IF EXISTS SANDBOX.DS.EXAMPLE_MODEL2CENTERS_SCORE_DECILE_20200225;
CREATE temp TABLE  SANDBOX.DS.EXAMPLE_MODEL2CENTERS_SCORE_DECILE_20200225 AS 
select *, case 
...
from SANDBOX.DS.EXAMPLE_MODEL2CENTERS_SCORE_20200225;



select NATIONAL_decile,count(*)
from EXAMPLE_MODEL2CENTERS_SCORE_DECILE_20200225
group by NATIONAL_decile
order by NATIONAL_decile;



DROP TABLE IF EXISTS EXAMPLE_MODEL2_TESTING_NATIONAL;
CREATE TEMP TABLE EXAMPLE_MODEL2_TESTING_NATIONAL AS
select A.*,B.NATIONAL_SCORE
from EXAMPLE_MODEL2_ALLCENTERS_UNIQUE_ID A
JOIN (SELECT UNIQUE_ID,NATIONAL_SCORE,NATIONAL_decile FROM EXAMPLE_MODEL2CENTERS_SCORE_DECILE_20200225) B
